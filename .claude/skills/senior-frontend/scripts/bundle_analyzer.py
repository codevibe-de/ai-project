...
def analyze_dependencies(package_json: dict) -> dict:
    ...
    if "prop-types" in deps and ("typescript" in dev_deps or "@types/react" in dev_deps):
        warnings.append(
            {"package": "prop-types", "type": "redundant", "message": "prop-types is redundant when using TypeScript"}
        )
...
def check_nextjs_config(project_dir: Path) -> dict:
    ...
    config_paths = [project_dir / "next.config.js", project_dir / "next.config.mjs", project_dir / "next.config.ts"]
                ...
                try:
                    ...
                    for pattern, message in patterns_to_check:
                        if re.search(pattern, content):
                            issues.append({"file": str(file_path.relative_to(project_dir)), "issue": message})
                except Exception:
                    continue
...
def calculate_score(analysis: dict) -> tuple[int, str]:
    ...
    score -= len([w for w in analysis["dependencies"]["warnings"] if w.get("type") == "dev_in_production"]) * 5
...
def main():
    parser = argparse.ArgumentParser(description="Analyze frontend project for bundle optimization opportunities")
    parser.add_argument(
        "project_dir", nargs="?", default=".", help="Project directory to analyze (default: current directory)"
    )
    parser.add_argument("--json", action="store_true", help="Output in JSON format")
    parser.add_argument("--verbose", "-v", action="store_true", help="Include detailed import analysis")
